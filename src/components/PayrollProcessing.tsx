...
import { Calculator, Download, Play, Eye, Loader2 } from "lucide-react";
...
const PayrollProcessing = () => {
  ...
  const handlePreviewCalculation = async () => {
    ...
    try {
      const preview = employees.map(employee => {
        try {
          return calculatePayroll(employee.id, selectedMonth, parseInt(selectedYear));
        } catch (error) {
          console.error(`Error calculating payroll for employee ${employee.id}:`, error);
          showErrorToast(`Error calculating payroll for ${employee.name}`, error instanceof Error ? error.message : 'Unknown error');
          return null;
        }
      }).filter(Boolean);
      ...
      setPreviewMode(true);
      showSuccessToast('Payroll preview generated successfully', `${preview.length} employee records calculated`);
    } catch (error) {
      console.error('Error generating payroll preview:', error);
      showErrorToast('Failed to generate payroll preview', 'Please check employee data and try again');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleProcessPayroll = async () => {
    ...
    try {
      ...
      setPreviewData([]);
      showSuccessToast('Payroll processed successfully', `Payroll for ${months.find(m => m.value === selectedMonth)?.label} ${selectedYear} has been processed`);
    } catch (error) {
      console.error('Error processing payroll:', error);
      showErrorToast('Failed to process payroll', 'Please try again or contact support');
    } finally {
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div className="flex gap-2">
              ...
              <Button 
                ...
                disabled={isProcessing || (previewMode && previewData.length === 0)}
              >
